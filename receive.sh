...
#timeout "${6}" /usr/local/bin/rtl_fm  -d "${DEVICE}" -f "${2}"M -s 60k -g "${RECEIVE_GAIN}" -E wav -E deemp -F 9 - | /usr/bin/sox -t wav - ${NOAA_AUDIO}/audio/"${3}".wav rate 11025
#echo timeout "${6}" /usr/local/bin/rtl_fm -f "${2}"M -s 60k -g 100 -E wav -E deemp -F 9 - 
#echo timeout "${6}" /usr/bin/rtl_fm -g 50  -d "${RTL_DEVICE}" -f "${2}"M -s 60k -E wav -E deemp -F 9 - \| /usr/bin/sox -t raw -e signed -c 1 -b 16 -r 60000 - ${NOAA_AUDIO}/audio/"${3}".wav rate 11025
#timeout "${6}" /usr/bin/rtl_fm -g 12  -d "${RTL_DEVICE}" -f "${2}"M -s 60k -E wav -E deemp -F 9 - | /usr/bin/sox -t raw -e signed -c 1 -b 16 -r 60000 - ${NOAA_AUDIO}/audio/"${3}".wav rate 11025
/usr/sbin/uhubctl -l 1-1 -p 2 -a 0
sleep 2
/usr/sbin/uhubctl -l 1-1 -p 2 -a 1
sleep 5
echo timeout "${6}" /usr/bin/rtl_fm  -g "${RECEIVE_GAIN}"  -f "${2}"M -s 60k -E wav -E deemp -F 9 - | /usr/bin/sox -t raw -e signed -c 1 -b 16 -r 60000 - ${NOAA_AUDIO}/audio/"${3}".wav rate 11025
timeout "${6}" /usr/bin/rtl_fm  -g "${RECEIVE_GAIN}"  -f "${2}"M -s 60k -E wav -E deemp -F 9 - | /usr/bin/sox -t raw -e signed -c 1 -b 16 -r 60000 - ${NOAA_AUDIO}/audio/"${3}".wav rate 11025
sleep 10
